...
document.addEventListener('DOMContentLoaded', function() {
    // Переменная для select элемента
    var selectEl = document.querySelector('.woocommerce-ordering .orderby');

    // Обработчик события при изменении значения select элемента
    selectEl.addEventListener('change', function() {
        // Получаем выбранное значение категории
        var selectedCategory = this.value;
        
        // Получаем все контейнеры продуктов
        var productContainers = document.querySelectorAll('.col-xl-4.col-sm-6');
        
        // Получаем элементы title-area для фильтрации
        var titleElements = document.querySelectorAll('.title-area');

        // Цикл по всем контейнерам продуктов и их фильтрация
        productContainers.forEach(function(container) {
            // Получаем элемент, содержащий данные о категории
            var product = container.querySelector('.th-product .product-img');

            // Проверяем, соответствует ли категория продукта выбранной.
            // Если да, отображаем его, если нет - скрываем
            if (selectedCategory === 'menu_order' || product.dataset.name === selectedCategory) {
        ...
        // Цикл по элементам title-area, аналогичный фильтрации продуктов
        titleElements.forEach(function(title) {
            // Проверяем, соответствует ли категория title выбранной
            if (selectedCategory === 'menu_order' || title.dataset.name === selectedCategory) {
                title.style.display = '';
            } else {
